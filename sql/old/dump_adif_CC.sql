...
CREATE VIEW adif_contable.vw_cc_obras_servicios AS 
SELECT 
    co.id_proveedor,
...
FROM adif_contable.comprobante AS c 
INNER JOIN adif_contable.tipo_comprobante AS tc 
    ON c.id_tipo_comprobante = tc.id
LEFT JOIN adif_contable.letra_comprobante AS lc 
...
LEFT JOIN adif_contable.orden_pago AS op 
...
WHERE c.fecha_baja IS NULL
    ...
    AND NOT EXISTS (SELECT 1 
                    FROM adif_contable.comprobante_ajuste AS ca
                    WHERE c.id = ca.id_comprobante);
$$
DELIMITER ;

DELIMITER $$
DROP VIEW IF EXISTS adif_contable.vw_cc_pagos_parciales$$
